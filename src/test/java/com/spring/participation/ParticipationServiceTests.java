...
public class ParticipationServiceTests {
    ...
    // 기존에 DB에 존재하는 회원 ID 가져오기
    private Long getExistingMemberId() {
        return memberRepository.findAll()
                .stream()
                .findFirst()
                .orElseThrow(() ->
                        new IllegalStateException("테스트에 사용할 회원이 없습니다."))
                .getMemberId();
    }
    ...
    private Long createTestRunning() {
        ...
        // 기존 회원 조회
        Long memberId = getExistingMemberId();
        ...
        runningDTO.setMemberId(memberId);
